...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const login = async (username: string, password: string, country: 'egypt' | 'libya') => {
    ...
    const userData: User = {
      ...
      name: country === 'egypt' ? 'Egypt Manager' : 'Libya Manager',
      username: username,
      role: 'admin',
      country: country,
    };
  ...
  const loginDirect = async (country: 'egypt' | 'libya') => {
    ...
    // Notify other context providers that auth state has changed
    window.dispatchEvent(new CustomEvent('auth:login', { detail: { country, user: userData } }));
  };
